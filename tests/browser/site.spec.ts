...
test('carousel advances and pauses only while the carousel is hovered', async ({ page }) => {
  ...
  const progressScale = () => progress.evaluate((bar) => {
  ...
  expect(duringReset).toBeLessThan(beforeHover);
  await seekProgress(220);
  expect(await progressScale()).toBeLessThan(0.01);
